...
<template>
  <div class="flex flex-col sm:flex-row sm:items-center gap-0 sm:gap-3 py-2" role="listitem">
    <div class="flex items-center gap-2 sm:gap-3 mb-2 sm:mb-0">
      ...
      <div class="flex-1 sm:flex-initial sm:min-w-0 relative">
        ...
        <input
          ...
          @blur="onBlur"
        />
        <p v-if="row.status === 'error' && row.errorMessage" :id="`error-${row.id}`" class="absolute left-0 top-full mt-1 text-xs sm:text-sm text-red-600 max-w-full" role="alert" :title="row.errorMessage">
          {{ row.errorMessage }}
        </p>
      </div>
    </div>

    <div class="flex-shrink-0 sm:w-48 flex items-center sm:justify-end gap-2 pl-9 sm:pl-0 min-h-[2rem]" role="status" :aria-live="row.status === 'loading' ? 'polite' : 'off'">
      <template v-if="row.status === 'loading'">
        ...
        <span class="text-xs sm:text-sm text-gray-500">Searching…</span>
      </template>
